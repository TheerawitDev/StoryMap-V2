        ...
        <section className="py-20 bg-gray-50">
            <div className="container mx-auto px-4">
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
                    <div>
                        <span className="text-primary font-semibold tracking-wider uppercase text-sm">{t('badge')}</span>
                        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-2">{t('title')}</h2>
                    </div>
                    <Link href="/explore" className="text-gray-600 hover:text-primary font-medium hover:underline flex items-center gap-2 group transition-colors">
                        {t('viewAll')} <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                    </Link>
                </div>
